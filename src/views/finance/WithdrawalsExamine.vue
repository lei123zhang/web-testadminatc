    ...
    <Card>
          ...
          刷新
				...
				<Row class="functionWrapper">
					<div class="btnsWrapper clearfix ">
						<Button type="info" @click="passAll">一键审核通过</Button>
      ...
      <Row class="pageWrapper">
        ...
        @on-ok="confrimPass">
			...
			<Modal
				v-model="passAllModal"
				title="一键审核通过"
				@on-ok="ifPassAll(true)"
				@on-cancel="$Message.info('已取消！')"
			>
				是否通过选中的<span style="color: #ec0909">{{ selectedNumArr.length }}</span>项
			</Modal>
...
export default{
	...
	created(){
		if(!!Cookies.get('financePage')) {
			this.currentPageIndex = this.filterSearch.pageNo = Cookies.get('financePage')*1;
		}
		this.refreshPage(this.filterSearch);
