<template>
	<div class="home">
		<Row type="flex" justify="space-between" class="dashBoard">
			...
			<div class="boardCol">
				<Card>
					...
					<div class="cardBody">
						<p>法币交易总交易量</p>
						<p class="tradeTotalNum">{{ otcChart.borderInfo.amount | cutNumber }}</p>
						<p>总交易额 <span>{{ otcChart.borderInfo.money | cutNumber}}</span></p>
						<p>总手续费 <span class="fee">{{ otcChart.borderInfo.fee | cutNumber}}</span></p>
					...
					<div class="cardFooterWrapper">
						<Row class="cardFooter">
							<Col span="11" offset="1">
								<p>昨日交易量 <span>{{ otcChart.borderInfo.yesterdayAmount | cutNumber }}</span></p>
							</Col>
							<Col span="11" offset="1">
								<p>昨日手续费 <span>{{ otcChart.borderInfo.yesterdayFee | cutNumber }}</span></p>
							</Col>
						</Row>
					</div>
				</Card>
			...
			<div class="boardCol">
				<Card>
					...
					<div class="cardBody">
						<p>币币交易总交易量</p>
						<p class="tradeTotalNum">{{ exchangeChart.boardInfo.amount | cutNumber }}</p>
						<p style="line-height: 60px">总手续费 <span class="fee">{{ exchangeChart.boardInfo.fee | cutNumber }}</span></p>
						<div class="selectWrapper">
							<Select v-model="exchangeChart.board.unit" size="small" @on-change="switchExchangeBoard">
					...
					<div class="cardFooterWrapper">
						<Row class="cardFooter" justify="space-around">
							<Col span="11">
								<p>昨日交易量 <span>{{ exchangeChart.boardInfo.yesterdayAmount | cutNumber }}</span></p>
							</Col>
							<Col span="11">
								<p>昨日手续费 <span>{{ exchangeChart.boardInfo.yesterdayFee | cutNumber }}</span></p>
							</Col>
						</Row>
					</div>
				</Card>
		...
		<Row class="register mainChart">
			<Card>
				<h3>注册、认证对比图<span> （查询日期不超过30天）</span></h3>
				<Row class="chartFunction">
					<Col span="3" offset="15">
						<span 
							:class="{ active: index === registerChart.timeRange.active }"
							v-for="(time, index) in registerChart.timeRange.time" 
							:key="time"
							@click="selectTime(index, registerChart)"
							>{{ time }}</span>
					</Col>
					<Col span="5">
		...
		<Row class="otcVolume mainChart">
			<Card>
				<h3>法币成交量趋势图<span> （查询日期不超过30天）</span></h3>
				<Row class="chartFunction">
					<Col span="3" offset="15">
						<span 
							:class="{ active: index === otcChart.timeRange.active }"
							v-for="(time, index) in otcChart.timeRange.time" 
							:key="time"
							@click="selectTime(index, otcChart)"
							>{{ time }}</span>
					</Col>
					<Col span="5">
		...
		<Row class="exchangeVolume mainChart">
			<Card>
				...
				<!-- <Row class="dataEmpty" v-if="!exchangeChart.chartData.length">
					<p>暂无数据！</p>
				</Row> -->
			</Card>
		</Row>
...
import { allExchangeCoin, memberBoard, memberChart, otcChart, coinChart, otcBoard, coinBoard,
allOtcCoin, allBaseCoin, allExchangeUnits } from '@/service/getData';
...
export default {
		data() {
			return {
				...
				registerChart: {
					...
					searchDate: {
						startDate: dtime(Date.now()-24*60*60000*3).format('YYYY-MM-DD'),
				...
				exchangeChart: {
					...
					searchDate: {
						// startDate: '2018-3-20',
						// endDate: '2018-4-19',
						startDate: dtime(Date.now()-24*60*60000*7).format('YYYY-MM-DD'),
						endDate: dtime(Date.now()-24*60*60000).format('YYYY-MM-DD'),
						baseSymbol: '',//交易结算币
						coinSymbols: []//平台币种集合
		...
		methods: {
			switchExchangeBoard() {
				coinBoard(this.exchangeChart.board)
				.then(res => {
					if(!res.code) {
						this.exchangeChart.boardInfo = res.data;
						console.log(this.exchangeChart.boardInfo);
			...
			limitTime(time, obj) {
				let limitTime = dtime(time[1])['0'].getTime() - dtime(time[0])['0'].getTime();
			...
			exchangeChartNum(sourceData, obj) {
				this.chartNum = new G2.Chart({
							container: obj.ref, 
							forceFit: true
						});
				this.chartNum.interval().position(`时间*${obj.type}`).color('币种').adjust('dodge');
				let legendArr = [];
				this.chartNum.filter('币种', val => {
						if(legendArr.indexOf(val)<0) legendArr.push(val);
					if(legendArr.length>1) return (val===legendArr[0] || val===legendArr[1]);
					else return val;
				})
				this.chartNum.render();
			},
			exchangeChartMoney(sourceData, obj) {
				this.chartMoney = new G2.Chart({
							container: obj.ref, 
							forceFit: true,
						});
						
				this.chartMoney.interval().position(`时间*${obj.type}`).color('币种').adjust('dodge');
				this.chartMoney.render();
			},
			formatDataSet(data) {
			...
			exchangeChartFn(searchObj) {
				coinChart(searchObj)
					.then(res => {
					if(!res.code) {
						let data = this.formatDataSet(res.data);
						this.chartMoney.changeData(data);
						this.chartNum.changeData(data);
					}else this.$Message.error(res.message);
				})
				.catch(err => console.log(err))
			},
		...
		filters: {
			cutNumber(val) {
				...
				if(val >1000) {
					let dot = val/1000;
					if(String(dot).length>5) {
						return dot.toFixed(5)+'K'
					}else return val;
				} else {
					let isDot = Math.trunc(val) !== val;
					if(isDot) {
						if(String(val).split('.')[1].length>6) {
		...
		mounted() {
			this.$nextTick(() => {
				this.exchangeChartNum([], { ref: this.$refs.exchangeNumChart, type: '交易量' });
				this.exchangeChartMoney([], { ref: this.$refs.exchangeMoneyChart, type: '交易额' });
